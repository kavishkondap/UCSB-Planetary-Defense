...
if (exists(name + '.bin')==False):
    checkpoint_2 = np.zeros ((NOI, N*N* frn))
    for index in range(NOI):
        ...
        vals = np.array (y_data[index] * temparr[...,None] * alpha * E[index] * 1/(4*np.pi) * 1/(np.sqrt(2*np.pi)*sigma[index]) * alpha_a[...,None])
        for i in range (vals.shape [0]):
            for j in range (vals.shape [1]):
                for k in range (vals.shape [2]):
                    print (i, j, k, vals[i, j, k])
        checkpoint_2[index] = vals.ravel ()
        zarray2[:,:,:] += y_data[index] * temparr[...,None] * alpha * E[index] * 1/(4*np.pi) * 1/(np.sqrt(2*np.pi)*sigma[index]) * alpha_a[...,None]
        print (index)
    df = pd.DataFrame (checkpoint_2)
    df.to_csv ('checkpoint2_cpu.csv')
    zarray2 += 1
