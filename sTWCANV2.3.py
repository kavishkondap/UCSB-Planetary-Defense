...
@numba.guvectorize (['(int16[:, :, :], float32 [:, :], int32, float32 [:], float32 [:], int32, int32, float32 [:,:], int32, float32 [:], int32, float32 [:], int32, float32 [:], float32 [:], int32)'], '(n),(n)->(n)', target = 'cuda')#assign a thread on GPU to each value in zarray2
# # @numba.guvectorize(['void(float32[:], float32[:], float32[:])'], '(n),(n)->(n)', nopython=True, target='cuda')
def computation (zarray2, alpha_a, NOI, x_gpu, y_gpu, rnge, increment, distarray, N, zburst_gpu, b, y_data, alpha, E_gpu, sigma_gpu, frn):
    # index = cuda.grid (1)
    ...
    if index < NOI:
        ...
        for i in range (len(temparr)):
            for j in range (len (temparr[0])):
                ...
                val = (dist_from_center*alpha_a[i, j]*E_gpu[index])/(sqrt(2*pi)*sigma_gpu[index])
