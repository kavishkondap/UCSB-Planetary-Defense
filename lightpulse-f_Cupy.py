...
def makeblasts(shr_name, index): #this function calculates and adds the lightpulse for any blast at the appropriate time in the simulation.
    if index < NOI:
        ...
        temparr = 1/temparr 
        alpha_a = b*cp.exp(a*temparr) #Atmospheric absorption
        newarr = y_data[index] #this is a gaussian modeled such that the peak is at the time of burst for any burst
        start = cp.min(cp.nonzero(newarr)) #choose stqart and end points for the gaussian as a lot of the data is 0
        end = cp.max(cp.nonzero(newarr))
        newarr = newarr[start:end] # clip the array
        arr = newarr * temparr[...,None] * alpha * E[index] * 1/(4*cp.pi) * 1/(cp.sqrt(2*cp.pi)*sigma[index]) * alpha_a[...,None] #this is the vectorized lightpulse calculated that needs to be added to the main array 
        zarr = cp.ndarray((N,N,frn),dtype = dtype, buffer = existing_shm.buf)   #extract array from shared memory     
...
def update_plot1(i): #this makes the plot we see in the final result. There is a faster way to do this but will change it once we speed up the calculations
    ...
    if (i)/10 - 5 < 0:
        ax3.plot(cp.linspace(-5,i/fps - 5,i), maxpower[0:i],label = 'Max Power')
    else:
        ax3.plot(cp.linspace(-5,i/fps - 5,i), maxpower[0:i],label = 'Max Power')
    ax4.cla()
...
def makearray(shr_name,i): #do integrals on the power array to make energy array (don't need a semaphore or a lock in this case as each calculation is done at a different third index)
    if i < frn:
        existing_shm=shared_memory.SharedMemory(name = shr_name)
        earr = cp.ndarray((N,N,frn),dtype = dtype, buffer=existing_shm.buf)
        temparr = zarray2[:,:,:i]-1
        arr = cp.sum(temparr,axis=2)*1/fps
        earr[:,:,i] = arr
        print('done w'+str(i))
        del arr
        del temparr
...
if __name__ == '__main__':
    ...
    xticks = cp.linspace(xcenter-rnge,xcenter+rnge,5,endpoint = True) #tick marks for the plots
    ...
    x = cp.delete(x,[0,1,2]) #delete the non-numeric data from each "array"
    y = cp.delete(y,[0,1,2])
    sigma = df['Sigma for Gaussian (in time) optical pulse power (s)'].to_numpy()
    sigma = cp.delete(sigma,[0,1,2])
    E = df['OUTPUT.28'].to_numpy()
    E = cp.delete(E,[0,1,2])
    zburst = df['OUTPUT.22'].to_numpy()
    times = df['OUTPU'].to_numpy()
    zburst = cp.delete(zburst,[0,1,2])
    times = cp.delete(times,[0,1,2])
    ...
    hundredpower = cp.delete(hundredpower,[0,1,2])
    times = times[:NOI]
    mindisp = times.min()
    ...
    y_data = []
    zarray2 = cp.zeros((N,N,frn),dtype = dtype)
    zarray2[:,:,:]=0
    E = E * 4.184 * 10**12
    ...
    params = cp.delete(params, [0,1,2,4,5])
    params = params[:2]
    a = params[0]
    b = params[1]
    ...
    if (exists(name + '_lightpulse.bin')==False): #this checks if there already is data written to the disk in which case it avoids doing all the calculations again. 
        ...
        arr = cp.ndarray(zarray2.shape,dtype = zarray2.dtype,buffer = shm.buf)
        arr[:,:,:] = zarray2[:,:,:]
        while starting < (NOI):
            jobs = []
    ...
    order = math.ceil(cp.log10(zarray2.max())) #some numbers for the plot
    ...
    Energyarray = cp.zeros((N,N,frn),dtype=dtype) # Energy array (Right side top plot)
    shm = shared_memory.SharedMemory(create = True, size = Energyarray.nbytes) #generate shared memory for the energy array
    arr = cp.ndarray(Energyarray.shape,dtype = Energyarray.dtype,buffer = shm.buf) 
    arr[:,:,:] = Energyarray[:,:,:]
    jobs=[]
    starting = 1
    ...
    count,bins_count = cp.histogram(overallmaxarray, bins = 100) # generates the bottom right most plot for the simulation
    print(fps)
    pdf = count/sum(count)
    pdf = cp.flip(pdf)
    cdf = cp.cumsum(pdf)
