...
def computation (a, NOI, x_gpu, y_gpu, rnge, increment, distarray, N, zburst_gpu, b, y_data, alpha, E_gpu, sigma_gpu, zarray2,e,pi,index, tracker, counter):
    print ("CALLING COMP")
    i = cuda.threadIdx.x
    j = cuda.blockIdx.x
    k = cuda.blockIdx.y
...
a=["100m_in_2000_1d","2","2","200","THIS WAS THE ERROR","100m_in_2000_1d","0","0","100","2000","100m","in","2000-Fragments(1Day)"]
...
if (exists(name + '1.bin')==False):
    ...
    counter = np.zeros ((2))
    counter [1] = 10101010
    counter = cuda.to_device (counter)
    checkpoint1 = zarray2.ravel ()
    df = pd.DataFrame (checkpoint1, columns=['checkpoint1'])
    tracker = cuda.to_device (np.ones ((N, N, frn)))
    df.to_csv ('checkpoint1_gpu.csv')
    for index in range(NOI):
        y_data_gpu = cuda.to_device(np.array (y_data[index], dtype = np.float32))
        computation[blocks_per_grid, threads_per_block](a, NOI, x_gpu, y_gpu, rnge, increment, distarray, N, zburst_gpu, b, y_data_gpu, alpha, E_gpu, sigma_gpu, zarray2_gpu,e,pi,index, tracker, counter)
        print("Called COMP ", index)
    tracker_cpu = tracker.copy_to_host ()
    # print (tracker_cpu)
    counter = counter.copy_to_host()
    print ("COUNTER: ", counter[0])
    zarray2 = zarray2_gpu.copy_to_host()
    ravel_tracker = {}
    for i in range (tracker_cpu.shape [0]):
        ravel_tracker[str (i)] = tracker_cpu[i].ravel ()
    checkpoint2 = ravel_tracker
    df = pd.DataFrame (checkpoint2, columns= [str(i) for i in range (tracker_cpu.shape [0])])
    df.to_csv ('checkpoint2_gpu.csv')
    zarray2 += 1
