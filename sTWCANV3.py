...
@cuda.jit#@numba.vectorized 
#assign a thread on GPU to each value in zarray2
def computation (zarray2, NOI, x_gpu, y_gpu, rnge, increment, distarray, N, zburst_gpu, b, y_data, alpha, E_gpu, sigma_gpu):
    # index = cuda.grid (1)
    xPos = cuda.threadIdx.x
    yPos = cuda.threadIdx.y
    zPos = cuda.threadIdx.z

    if xPos < zarray2.shape [0] and yPos < zarray2.shape [1] and zPos < zarray2.shape [2]:
        xpos = int((x_gpu[index]+rnge)/increment)
        ypos = int((y_gpu[index]+rnge)/increment)
        temparr = distarray[-(N+ypos):,N-xpos:]
        temparr = temparr[:N,:N]
        # print (temparr.shape)
        # for i in range (temparr.shape (0)):
        #     for j in range (temparr.shape (1)):
        ...
        zarray2[location[0], location[1], location[3]] += y_data[index] * temparr[...,None] * alpha * E_gpu[index] * 1/(4*np.pi) * 1/(np.sqrt(2*np.pi)*sigma_gpu[index]) * alpha_a[...,None]
...
if (exists(name + '.bin')==False):
    ...
    for index in range(NOI):
        ...
        computation [blockspergrid, threadsperblock](zarray2, NOI, x, y, rnge, increment, distarray, N, zburst, b, y_data, alpha, E, sigma)
        # computation [blockspergrid, threadsperblock](zarray2, NOI, x_gpu, y_gpu, rnge, increment, distarray, N, zburst_gpu, b, y_data, alpha, E_gpu, sigma_gpu)
