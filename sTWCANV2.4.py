...
# c= 1997
# d = 500
@numba.guvectorize (['(float32 [:, :], int32, float32 [:], float32 [:], int32, int32, float32 [:,:], int32, float32 [:], int32, float32 [:], int32, float32 [:], float32 [:], int16[:, :, :])'], '(a, a), (), (c), (c), (), (), (d, d), (), (c), (), (b, b), (), (c) -> (a, a, b)', target = 'cuda')#assign a thread on GPU to each value in zarray2
# # @numba.guvectorize(['void(float32[:], float32[:], float32[:])'], '(n),(n)->(n)', nopython=True, target='cuda')
def computation (alpha_a, NOI, x_gpu, y_gpu, rnge, increment, distarray, N, zburst_gpu, b, y_data, alpha, E_gpu, sigma_gpu, zarray2):
    # index = cuda.grid (1)
    ...
    if index < NOI:
        ...
        for i in range (len(temparr)):
            for j in range (len (temparr[0])):
                ...
                dist_from_center = sqrt (xpos**2+ypos**2)
                val = (y_data[index] *dist_from_center*alpha_a[i, j]*E_gpu[index])/(sqrt(2*pi)*sigma_gpu[index])
                zarray2 [i, j, index] += val
        #a+b
...
if (exists(name + '.bin')==False):
    ...
    for index in range(NOI):
        ...
        print ("SIGMAGPU: ", sigma_gpu.shape)
        print ("ALPHA A: ", alpha_a.shape)
        print ("X: ", x_gpu.shape)
        print ("Y: ", y_gpu.shape)
        print ("DISTARR: ", distarray.shape)
        print ("ZBURST GPU: ", zburst_gpu.shape)

        # computation [blockspergrid, threadsperblock](zarray2, alpha_a, NOI, x, y, rnge, increment, distarray, N, zburst, b, y_data, alpha, E, sigma)
        ...
        computation (alpha_a, NOI, x_gpu, y_gpu, rnge, increment, distarray, N, zburst_gpu, b, y_data, alpha, E_gpu, sigma_gpu, zarray2, 0)
        # xpos = int((x_gpu[index]+rnge)/increment)
        # ypos = int((y_gpu[index]+rnge)/increment)
        # temparr = distarray[-(N+ypos):,N-xpos:]
        # temparr = temparr[:N,:N]
