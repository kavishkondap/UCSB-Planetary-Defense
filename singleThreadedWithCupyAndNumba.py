...
a=["100m_in_2000_1d","500","2000","200","THIS WAS THE ERROR","100m_in_2000_1d","0","0","100","2000","100m","in","2000-Fragments(1Day)"]
...
if mindisp < 0:
    ...
    times = times - mindisp + 5
zarray2 = cp.zeros((N,N,frn), dtype=np.int16)
...
params = cp.array (params[:2], dtype = np.float64)
a = params[0]
b = params[1]
begin_time = datetime.datetime.now()
print('starting calculations for' + name)
if (exists(name + '.bin')==False):
    # splits = np.linspace (0, len (x)-3, num=8, dtype = int)
    # print (splits)
    # # splits[7] = len (x)-3
    # # print (splits)
    ...
    #         firstIndex = 0
    #     else:
    #         firstIndex = splits[i-1]
    #     x_gpu = cp.array (np.delete(x,[0,1,2])[firstIndex:splits[i]], dtype = np.float64) #delete the non-numeric data from each "array"
    #     y_gpu = cp.array (np.delete(y,[0,1,2])[firstIndex:splits[i]], dtype = np.float64) #delete the non-numeric data from each "array"
    #     sigma_gpu = cp.array (np.delete(sigma,[0,1,2])[firstIndex:splits[i]], dtype = np.float64) #delete the non-numeric data from each "array"
    #     E_gpu = cp.array (np.delete(E,[0,1,2])[firstIndex:splits[i]], dtype = np.float64) #delete the non-numeric data from each "array"
    #     zburst_gpu = cp.array (np.delete(zburst,[0,1,2])[firstIndex:splits[i]], dtype = np.float64) #delete the non-numeric data from each "array"
    ...
    x_gpu = cp.array (np.delete(x,[0,1,2]), dtype = np.float64) #delete the non-numeric data from each "array"
    y_gpu = cp.array (np.delete(y,[0,1,2]), dtype = np.float64) #delete the non-numeric data from each "array"
    sigma_gpu = cp.array (np.delete(sigma,[0,1,2]), dtype = np.float64) #delete the non-numeric data from each "array"
    E_gpu = cp.array (np.delete(E,[0,1,2]), dtype = np.float64) #delete the non-numeric data from each "array"
    zburst_gpu = cp.array (np.delete(zburst,[0,1,2]), dtype = np.float64) #delete the non-numeric data from each "array"
    times_gpu = cp.array (np.delete(times,[0,1,2]), dtype = np.float64) #delete the non-numeric data from each "array"
    hundredpower_gpu = cp.array (np.delete(hundredpower,[0,1,2]), dtype = np.float64) #delete the non-numeric data from each "array"

    @cuda.jit
    def calculation (NOI, x_gpu, y_gpu, rnge, increment, distarray, N, zburst_gpu, alpha, E_gpu, sigma_gpu, zarray2):
        i, j, k = cuda.grid (3)
    ...
    calculation [blockspergrid, threadsperblock] (NOI, x_gpu, y_gpu, rnge, increment, distarray, N, zburst_gpu, alpha, E_gpu, sigma_gpu, zarray2) 
    zarray2 += 1
