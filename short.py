...
lock = mp.Lock()
dtype = np.float64
def makeblasts(shr_name, index, NOI, x, rnge, increment, y, distarray, N, zburst, b, a, y_data, alpha, E, sigma, frn):
    if index < NOI:
        ...
        arr = newarr * temparr[...,None] * alpha * E[index] * 1/(4*np.pi) * 1/(np.sqrt(2*np.pi)*sigma[index]) * alpha_a[...,None] #slow
...
def makearray(shr_name,i, frn, N, zarray2, fps,):
    if i < frn:
        existing_shm=shared_memory.SharedMemory(name = shr_name)
...
#        a[12] = n
    ...
    if (exists(name + '_lightpulse.bin')==False):
        ...
        while starting < (NOI):
            ...
            for i in range(starting,starting+maxloadcalc):
                p = mp.Process(target=makeblasts,args=(shm.name,i, NOI, x, rnge, increment, y, distarray, N, zburst, b, a, y_data, alpha, E, sigma, frn ))
                jobs.append(p)
                p.start()
                starting += 1
            for z in jobs:
    ...
    while starting < frn:
        for i in range(starting, starting+maxload):
            p = mp.Process(target=makearray,args = (shm.name,i,frn, N, zarray2, fps,))
            starting += 1
            p.start()
            if starting == 1500:
                maxload = 10
